...
/**
 * Persistent customer dashboard chrome (sidebar + header) — wraps every page
 * under app/customer/ once a customer is signed in. Role-scoped per the
 ...
 * the customer's home is the Intent Search chat, not a dashboard of numbers
 * (no mockup in the reference folder showed a customer stats page).
 */

const NAV_ITEMS = [
  { href: "/customer/dashboard", label: "New Search", icon: SquarePen },
  { href: "/products", label: "Marketplace", icon: Store },
  { href: "/customer/chats", label: "Chats", icon: MessageSquare },
  { href: "/customer/orders", label: "History", icon: History },
] as const;

const NAV_ITEMS_BOTTOM = [{ href: "/customer/settings", label: "Settings", icon: SettingsIcon }] as const;
    ...
    <div className="flex h-dvh flex-col bg-cream">
      ...
      <header className="grid shrink-0 grid-cols-[auto_1fr_auto] items-center gap-4 border-b border-line bg-white px-4 py-3 sm:px-6 lg:px-10">
        <div className="flex items-center gap-2">
          ...
          <Link href="/customer/dashboard" className="font-alata text-lg tracking-tight text-ink sm:text-xl">
            VITALINK
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2 justify-self-end sm:gap-3">
          ...
          <AccountMenu name={customerName} badge="Customer" />
        </div>
      </header>
    ...
    <>
      {!bottomOnly && (
        <nav className="space-y-1">
          {NAV_ITEMS.map((item) => (
            <CustomerNavLink key={item.href} item={item} active={pathname?.startsWith(item.href) ?? false} />
          ))}
        </nav>
      )}
      {!topOnly && (
        <nav className="mt-8 space-y-1 border-t border-line pt-4 lg:mt-0 lg:border-t-0 lg:pt-0">
          {NAV_ITEMS_BOTTOM.map((item) => (
            <CustomerNavLink key={item.href} item={item} active={pathname === item.href} />
          ))}
        </nav>
      )}
    </>
