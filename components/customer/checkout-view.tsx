...
import type { CustomerOrderItem, CustomerDeliveryAddress } from "@/lib/api/customer-orders";
import { completeCheckoutAction } from "@/app/customer/checkout/actions";
 ...
 * model) into the flat CustomerDeliveryAddress shape completeCheckoutAction
 * still expects — that action is the mock order-completion flow (lib/api/
 * customer-orders.ts), unrelated to and out of scope for this pass's real
...
function toDeliveryAddress(address: CustomerAddress): CustomerDeliveryAddress {
  return {
 ...
 *
 * Delivery Address picks from the customer's saved address book
 * (lib/api/addresses.ts) rather than free-text fields typed fresh every
 * time — addresses are managed on Settings (components/customer/address-book.tsx). */
export function CheckoutView({ initialAddresses }: { initialAddresses: CustomerAddress[] }) {
  ...
  function handleSubmit() {
    ...
    const orderItems: CustomerOrderItem[] = items.map((item) => ({
    ...
    startTransition(async () => {
      ...
      clearCart();
      router.push(`/customer/checkout/success?orderId=${result.data.orderId}`);
    });
  }
          ...
          <div className="rounded-2xl border border-line bg-white p-5 sm:p-6">
            ...
            {initialAddresses.length === 0 ? (
              <div className="mt-4 rounded-xl border border-dashed border-line p-4 text-sm text-text-muted">
                ...
                <Link href="/customer/settings" className="font-medium text-verified hover:text-ink">
                  Add one in Settings
                </Link>{" "}
                before checking out.
              </div>
