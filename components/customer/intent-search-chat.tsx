 ...
 * /customer/dashboard (starts blank) and /customer/chats/[id] (resumes a saved
...
export function IntentSearchChat({
  ...
  function submit(query: string) {
    ...
    startTransition(async () => {
      ...
      if (!chat) {
        router.replace(`/customer/chats/${updatedChat.id}`);
      }
    });
  }
          ...
          {recentChats.length > 0 && (
            <div>
              <div className="flex items-center justify-between">
                ...
                <Link href="/customer/chats" className="flex items-center gap-0.5 text-xs font-medium text-verified hover:text-ink">
    ...
    <div className={cn("grid h-full grid-cols-1 gap-6", hasMultipleTurns && "lg:grid-cols-[280px_1fr]")}>
      ...
      <div className="flex min-w-0 flex-col">
        <div className="flex items-center justify-between border-b border-line pb-4 lg:hidden">
          <p className="text-sm font-semibold text-ink">Vitalink Intelligence</p>
          <Link href="/customer/dashboard" className="flex items-center gap-1.5 text-xs font-medium text-ink-soft hover:text-ink">
            <SquarePen className="size-3.5" aria-hidden />
            New Search
          </Link>
        </div>
