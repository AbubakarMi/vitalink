...
import { getAdminCustomerDetails } from "@/lib/api/admin/customers";
...
export default async function AdminCustomerDetailPage({ params }: PageProps) {
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-6">
      <Link href="/admin/customers" className="text-sm font-medium text-text-muted hover:text-ink">
        ← Customers
      </Link>
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-line bg-white p-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-6">
          ...
          <Stat label="Total Spent" value={`N${customer.totalSpent.toLocaleString("en-NG")}`} />
        </div>
      </div>
      ...
      <div>
        ...
        {orders.length === 0 ? (
          <p className="mt-3 text-sm text-text-muted">This customer hasn&apos;t placed any orders yet.</p>
        ) : (
