 ...
 * /buyer/chats/[id]. */
export default async function BuyerChatsPage({ searchParams }: PageProps) {
  await requireAccountType("buyer", "/buyer/chats");
  const params = await searchParams;
  const chats = await listChatsForBuyer();
    ...
    <div className="mx-auto max-w-2xl">
      ...
      {chats.length === 0 ? (
        <div className="mt-8 flex flex-col items-center rounded-2xl border border-dashed border-line py-16 text-center">
          ...
          <Link href="/buyer/dashboard" className="mt-4 rounded-xl bg-ink px-5 py-2.5 text-sm font-medium text-white hover:bg-ink/85">
            New Search
          </Link>
        </div>
      ) : (
...
function PageLink({ page, disabled, children }: { page: number; disabled: boolean; children: React.ReactNode }) {
  ...
  return (
    <Link href={`/buyer/chats?page=${page}`} className="rounded-lg border border-line px-3 py-1.5 text-sm text-ink hover:border-ink/40">
      {children}
    </Link>
  );
}
