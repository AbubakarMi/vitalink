 ...
 *
 * Allows Vendor sessions through too because /buyer/cart, /buyer/checkout,
 * and /buyer/orders are open to them — a vendor can buy on the marketplace
 * and track their own purchases like any other shopper (route-groups.ts's
 * isBuyerPathOpenToVendors). A vendor here only ever reaches one of those
 * three — every other /buyer/* page's own requireAccountType("buyer", ...)
 * call (no alsoAllow) redirects them away before render.
 *
 * Also renders through with NO session at all — /buyer/cart is guest-
 * accessible (route-groups.ts's isGuestAllowedBuyerPath, same reasoning as
 ...
 * reaches this far for that one allowed path — every other /buyer/* path
 ...
 * uses for anyone not signed in as a Customer, not the buyer account
 * dashboard.
 */
