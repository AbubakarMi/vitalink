 ...
 *
 * Unlike every other /buyer/* page, this one is deliberately guest-
 * accessible (route-groups.ts's isGuestAllowedBuyerPath, mirrored in
 * proxy.ts) — same as any real storefront, holding a cart doesn't need an
 * account, only paying for it does (app/buyer/checkout/page.tsx keeps the
 * hard requireAccountType). A signed-in session still has to be the right
 * role (Customer or Vendor — see route-groups.ts's isBuyerPathOpenToVendors)
 ...
 * instead of just bouncing them (components/buyer/checkout-cta.tsx).
 *
