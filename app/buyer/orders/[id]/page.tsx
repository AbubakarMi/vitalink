...
import { getBuyerOrderById, type BuyerOrderStatus } from "@/lib/api/buyer-orders";
import { VitalsStatusTrace, VitalsCancelledTrace } from "@/components/buyer/vitals-status-trace";
...
function stageIndex(status: BuyerOrderStatus): number {
...
export default async function BuyerOrderDetailPage({ params }: PageProps) {
  ...
  return (
    <div>
      <Link href="/buyer/orders" className="text-sm font-medium text-text-muted hover:text-ink">
        ← Order History
      </Link>
