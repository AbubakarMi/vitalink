...
import { getCustomerOrderById } from "@/lib/api/customer-orders";
import { VitalsStatusTrace } from "@/components/customer/vitals-status-trace";
 ...
 * too (route-groups.ts's isCustomerPathOpenToVendors) — the post-checkout links
 * below route a vendor to their own dashboard instead, since /customer/orders
 * and /customer/dashboard stay Customer-only. */
export default async function CheckoutSuccessPage({ searchParams }: PageProps) {
  const session = await requireAccountType("customer", "/customer/checkout", ["Vendor"]);
  const isVendor = session.accountType === "Vendor";
  const { orderId } = await searchParams;
  const order = orderId ? await getCustomerOrderById(orderId) : null;
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center py-16 text-center">
      ...
      <div className="mt-8 flex flex-col gap-3 sm:flex-row">
        ...
        <Link
          href={isVendor ? "/vendor/dashboard" : "/customer/dashboard"}
