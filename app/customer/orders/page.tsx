...
import { listOrdersForCustomer, type CustomerOrderStatus } from "@/lib/api/customer-orders";
import { StatusPill } from "@/components/customer/status-pill";

export const instant = false; // requireAccountType reads cookies — genuinely dynamic

const PAGE_SIZE = 8;
const STATUS_FILTERS: { value: CustomerOrderStatus | "all"; label: string }[] = [
...
/** Matches Frame 2018776196.pdf's "Order History" (the customer sidebar's
...
export default async function CustomerOrdersPage({ searchParams }: PageProps) {
  ...
  // history here too — see lib/auth/route-groups.ts's isCustomerPathOpenToVendors.
  await requireAccountType("customer", "/customer/orders", ["Vendor"]);
  const params = await searchParams;
  const orders = await listOrdersForCustomer();

  const query = (params.q ?? "").trim().toLowerCase();
  const statusFilter = (params.status ?? "all") as CustomerOrderStatus | "all";
                ...
                {pageItems.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="border-b border-line/70 last:border-b-0 hover:bg-cream/40">
                      ...
                      <td className="px-5 py-4">
                        <Link href={`/customer/orders/${order.id}`} className="font-medium text-verified hover:text-ink">
                          View
                        </Link>
                      </td>
                    </tr>
...
function PageLink({
  ...
  return (
    <Link href={`/customer/orders?${query.toString()}`} className="rounded-lg border border-line px-3 py-1.5 text-sm text-ink hover:border-ink/40">
      {children}
    </Link>
  );
}
