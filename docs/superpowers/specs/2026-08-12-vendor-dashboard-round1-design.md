# Vendor Dashboard, Round 1 (Overview / Global Inventory / New Product / Product Detail) — Design

Date: 2026-08-12
Status: Approved by user (brainstorming session), pending implementation plan
...
`app/vendor/*` ("no API yet"). The user supplied a folder of rough mockup exports
...
exists yet. **Orders** is a deferred round 2 (two near-duplicate mockups need
reconciling — see below — and Overview only needs a thin slice of order data, not the
full page). **Transactions, Analytics, Settings** have nav entries but no supplied
mockup for any of them — left as simple placeholder routes until designs exist, same as
the existing stub pattern for those routes today.
...
- Role badge reads "Seller" in some exports, "Vendor" in others — this app uses
  "Vendor" everywhere else (`AccountType.Vendor`, `/vendor` routes, the onboarding
  wizard's own copy); standardizing on "Vendor".
...
- Two near-duplicate order-list exports (`Vendor Oder.pdf` "Order History" vs.
  `Vendor Order Page.pdf` "Order Fulfilment") with a relabeled column
  ("QTY" vs "Number of Items") but otherwise identical data — moot for this round
  since Orders is deferred, but noted for round 2 so it isn't built twice.
...
- Stat tables show fabricated large numbers ("22,500 results", "1,465 products",
  "246 products"). Consistent with the existing `lib/api/products.ts` convention
  (`listProductsPaged`'s own comment on this exact point), real pagination/counts over
  the actual mock dataset size are used instead — never the mockup's fabricated figures.
...
(role-scoped, per the "components never cross role boundaries" rule in the frontend
architecture doc) providing:
...
This shell wraps every page under `app/vendor/` going forward, replacing the current
bare `{children}` passthrough.

**Data layer.** No `Product`, `Order`, or `Wallet` entity exists on the real backend
yet (confirmed in `docs/superpowers/specs/2026-08-06-vitalink-frontend-architecture-design.md`
— every `app/vendor/products*` and `app/vendor/orders` route is marked fully mocked).
Follows the existing mock/live seam pattern (`lib/api/products.ts`'s
`PRODUCTS_DATA_SOURCE`, `lib/api/mocks/vendor-profile-store.ts`'s in-memory store):
...
- New `lib/api/mocks/vendor-inventory-store.ts` — mutable, `globalThis`-pinned like
  ...
  more often than a real process restart would, which would otherwise wipe a vendor's
  data mid-session), keyed by vendor `userId`, seeded with a representative product set
  on first access per vendor.
- New `lib/api/vendor-products.ts`: `listProductsForVendor(userId)`,
  ...
  steps 1-2), `generateProductDetails(input)` (the "AI" step — a deterministic
  templated generator keyed off category/brand/model, not a real model call, per the
  round-1 scope decision), `publishVendorProduct(userId, id)`,
  `updateVendorProductStatus(userId, id, status)`.
- A minimal `lib/api/mocks/vendor-orders-store.ts` slice (a handful of seeded orders
  per vendor) so Overview's Recent Orders table and stat pill have something real to
  render — the full Orders page/filtering/pagination stays out of scope for round 2.
- Overview's Total Sales / Wallet Balance stay explicitly mocked placeholder figures,
  same honesty convention `vendor/dashboard/page.tsx` already uses today.
...
(detail — new route, distinct from the existing `products/[id]/edit/page.tsx`). All
call `requireAccountType("Vendor")` per the existing per-page DAL-guard convention.

## 3. Overview (`/vendor/dashboard`)

- Header: business name + verification badge (real, from `getVendorProfile()`).
- Stat row: Total Sales, Wallet Balance (both mocked placeholders, labeled honestly),
  Live Products (real count from `listProductsForVendor`), Low Stock Alerts (real,
  derived from `lowStockThreshold`). Styled with the onboarding shell's
  mono-label/serif-value treatment, not the mockup's plain sans-serif numbers.
...
- Recent Orders table (last 5-7) — reuses the same table primitive Global Inventory
  uses, backed by the minimal mock order slice, so all vendor tables read as one
  system.
...
- **Step 2 — Identification**: Product Name, Manufacturer, Model, Country of Origin,
  **plus Price, Promo Price, Stock, and Low-Stock Threshold**. The mockup's step 2
  subtitle promises "pricing, stock units, and reorder alert levels" but its visible
  fields only cover name/manufacturer/model/origin — those pricing/stock fields have to
  live somewhere, and folding them into step 2 (rather than inventing an ungrounded new
